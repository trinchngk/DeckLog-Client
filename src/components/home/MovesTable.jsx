      ...
      <table className='mt-4 w-full border-separate border-spacing-2'>
        <thead className='border border-slate-600 rounded-md'>
          <tr>
            ...
            <th className='border border-slate-600 rounded-md max-md:hidden'>Tags</th>
            <th className='border border-slate-600 rounded-md'>Status</th>
            <th className='border border-slate-600 rounded-md'>Started</th>
        ...
        <tbody>
          {moves.map((move, index) => (
            <tr key={move._id} className='h-8'>
              ...
              <td className='border border-slate-700 rounded-md text-center text-white max-md:hidden'>
                {move.desc.length > maxChars ? `${move.desc.slice(0, maxChars)}...` : move.desc}
              </td>
              <td className='border border-slate-700 rounded-md text-center text-white max-md:hidden'>
                {move.tags}
              </td>
              <td className="border border-slate-700 rounded-md text-center text-white place-items-center h-full">
                {move.finished ? (
                  <FaCheck className="text-2xl text-green-500" />
                ) : (
                  <MdConstruction className="text-2xl text-orange-500" />
                )}
              </td>
              <td className='border border-slate-700 rounded-md text-center text-white max-md:hidden'>
